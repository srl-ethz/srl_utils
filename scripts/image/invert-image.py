...
class ImageInverter:
    ...
    def setup_window(self):
        ...
        self.label = ttk.Label(
            self.root,
            text="Drag and drop an image file here\nto invert its colors.",
            justify=tk.CENTER,
        )
        self.label.pack(pady=20)

        self.chk_save_as = ttk.Checkbutton(
            self.root,
            text="Choose save location for inverted image",
            variable=self.save_var,
        )
        self.chk_save_as.pack(pady=10)

        self.get_from_clipboard = ttk.Button(
            self.root,
            text="Get Image from Clipboard",
            command=self.handle_image_from_clipboard,
        )
    ...
    def save_image(self, image, output_path):
        ...
        messagebox.showinfo(
            "Success", f"Inverted image saved to {output_path}"
        )
