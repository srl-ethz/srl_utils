...
cut_movie = cv2.VideoWriter(f"{folder}/cut{file_name}.mp4", fourcc, fps, (frame.shape[1], frame.shape[0]))
tracked_movie = cv2.VideoWriter(f"{folder}/tracked_{file_name}.mp4", fourcc, fps, (frame.shape[1], frame.shape[0]))
...
while cap.isOpened():
    ...
    if not found:
        cv2.putText(frame, "One of the objects not found", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255), 2)
    ...
    for i in range(N):
        ...
        curr_center = np.array([bboxes_new[i][0] + bboxes_new[i][2]/2, bboxes_new[i][1] + bboxes_new[i][3]/2])
