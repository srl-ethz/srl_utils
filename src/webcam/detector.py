    ...
    def detect_kpt(self, num:int): 
        ...
        if self.circles.size > 3:
            ...
            for i in range(len(x_list)):
                roi = get_ROI_coordinates((x_list[i], y_list[i]), imageSize, (region*r_list[i],region*r_list[i]) )
                local_gray_blured = gray_blured[int(roi[1]):int(roi[1]+roi[3]), int(roi[0]):int(roi[0]+roi[2])]
                local_circles = cv2.HoughCircles(local_gray_blured,cv2.HOUGH_GRADIENT, 1, self.min_dist,
                        param1=50,param2=self.param2,minRadius=self.min,maxRadius=self.max)
                if local_circles is not None:
                    for local_circle in local_circles[0,:]:
                        cx,cy = coord_xform_roi_to_img((local_circle[0], local_circle[1]), roi)
                        updated_x_list.append(cx)
                        updated_y_list.append(cy)
                        updated_r_list.append(local_circle[2])

            circles = np.array([np.stack([updated_x_list, updated_y_list, updated_r_list]).T])
            ...
            # t1 = time.time()
            circles = cv2.HoughCircles(gray_blured,cv2.HOUGH_GRADIENT, 1, self.min_dist,
                                param1=50,param2=self.param2,minRadius=self.min,maxRadius=self.max)
        ...
        for circle in circles[0,:]:
            ...
            roi = get_ROI_coordinates((cx, cy), img_shape, (1.1 * dia, 1.1 * dia))
            corner_x, corner_y, lh, lw = roi
            cropped_img = gray_blured[corner_y:corner_y+lw, corner_x:corner_x+lh]
        ...
        def get_center_from_ROI(roi_img):
            ...
            outer_mask = cv2.circle(outer_mask, center_approx, int(0.9 * rad_approx), (255,255,255), -1)
            ...
            outer_mask = np.zeros_like(img)
            outer_mask = cv2.circle(outer_mask, center_approx, int(0.8 * rad_approx), (255,255,255), -1)
            outer_mask = cv2.circle(outer_mask, center_approx, int(0.2 * rad_approx), (0,0,0), -1)
            ...
            if lines is not None:
                for line in lines:
                    ...
                    uiuiT = np.array([[   ui[0]**2 , ui[0]*ui[1]],\
                                      [ui[0]*ui[1] ,   ui[1]**2 ]])
        ...
        for res in results:
            cv2.circle(self.img, (int(res[0]), int(res[1])), 3, (0,0,255), -1)
